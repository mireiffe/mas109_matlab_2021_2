...
1. 행렬의 수학적 연산 `+`, `*`, `^`, `inv`
    ...
    INV2 = inv(A)
    ```
    행렬간 나눗셈은 정의되지 않습니다. 따라서, `A/B`와 같은 구문은 수학적으로는 정의할 수 없는 계산입니다. 그렇다면 다음과 같은 계산을 하면 오류 메시지가 나타날까요?
    ```matlab
    DIV = A / B
    ```
...
3. 행렬 요소별 연산 `.*`, `.^`, `./`
    ...
    EL_DIV = A ./ B     % elementwise division
...
```matlab
s = "hello"; x = exp(1);
disp(s, x);          % Wrong syntax
disp([s, x]);        % Possible syntax
```
